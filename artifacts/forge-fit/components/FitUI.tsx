...
export function Header({ eyebrow, title, subtitle, action, actionLogo = false, onAction, premiumLabel, premiumAction, premiumIcon = 'trophy-outline', premiumOwned = false, streak, streakLabel, onStreakPress, centered = false, lightBackground = false, showText = true }: { eyebrow?: string; title: string; subtitle?: string; action?: IconName; actionLogo?: boolean; onAction?: () => void; premiumLabel?: string; premiumAction?: () => void; premiumIcon?: IconName; premiumOwned?: boolean; streak?: number; streakLabel?: string; onStreakPress?: () => void; centered?: boolean; lightBackground?: boolean; showText?: boolean }) {
  const colors = useColors();
  ...
  return <View style={styles.header}>
    ...
    <View style={[styles.headerActions, centered ? styles.headerActionsCentered : null]}>
      {streak !== undefined ? <Pressable testID="header-streak" accessibilityRole="button" accessibilityLabel={`${streak} ${streakLabel ?? ''}`} onPress={onStreakPress ? () => { triggerHaptic(); onStreakPress(); } : undefined} style={({ pressed }) => [styles.streakPill, { backgroundColor: `${colors.orange}20`, borderColor: `${colors.orange}55`, opacity: pressed ? 0.72 : 1 }]}><Ionicons name="flame" size={15} color={colors.orange} /><Text style={[styles.streakValue, { color: colors.orange }]}>{streak}</Text>{streakLabel ? <Text style={[styles.streakLabel, { color: colors.orange }]}>{streakLabel}</Text> : null}</Pressable> : null}
