...
export default function PlanScreen() {
  ...
  React.useEffect(() => {
    if (workouts.length > 0 && !workouts.some((workout) => workout.day === activeDay)) {
      setActiveDay(workouts[0].day);
    }
  }, [activeDay, workouts]);

  if (!active) return <Screen><Header eyebrow={t('planEyebrow')} title={t('planTitle')} subtitle={t('planSubtitle')} /><EmptyState icon="barbell-outline" title={t('noWorkout')} text={t('createPlan')} /></Screen>;
