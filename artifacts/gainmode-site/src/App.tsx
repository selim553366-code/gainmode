    ...
    <div className="min-h-[100dvh] overflow-x-hidden bg-[#f1f5f9] text-[#17263c]">
      <header className="fixed left-0 right-0 top-0 z-50 px-3 pt-3 sm:px-6">
        <div className="container-wide flex h-[68px] items-center justify-between rounded-2xl border border-white/70 bg-white/85 px-4 shadow-[0_12px_32px_rgba(24,43,73,.08)] backdrop-blur-xl sm:px-6">
          <a href="#top" data-testid="link-logo" aria-label="GainMode home"><img src={wordmark} alt="GainMode" className="wordmark" width="142" height="22" /></a>
      ...
      <main id="top">
        <section className="relative min-h-[780px] overflow-hidden bg-[#d9effb] pt-32 sm:min-h-[850px] sm:pt-40">
          ...
          <div className="container-wide relative grid items-center gap-10 lg:grid-cols-[1fr_.86fr]">
            <div className="max-w-[630px]">
              ...
              <h1 className="reveal delay-1 display whitespace-pre-line text-[clamp(3.4rem,8vw,7.7rem)] font-extrabold leading-[.89] text-[#17263c]">{t.heroTitle}</h1>
        ...
        <section id="method" className="soft-grid bg-[#f1f5f9] py-24 sm:py-32">
          <div className="container-wide grid gap-14 lg:grid-cols-[.82fr_1.18fr] lg:gap-24">
            <Reveal><span className="eyebrow text-cyan-700">{t.methodKicker}</span><h2 className="display mt-5 whitespace-pre-line text-4xl font-extrabold leading-[.98] text-[#17263c] sm:text-6xl">{t.methodTitle}</h2><p className="mt-7 max-w-md text-base leading-7 text-slate-600">{t.methodBody}</p><div className="mt-10 flex items-center gap-3 text-sm font-bold text-slate-700"><div className="h-10 w-10 rounded-full bg-cyan-100 p-2.5 text-cyan-700"><Target size={20} /></div>{micro.methodNote}</div></Reveal>
        ...
        <section id="inside" className="bg-[#17263c] py-24 text-white sm:py-32">
          <div className="container-wide">
            ...
            <div id="fitbud" className="mt-16 grid gap-4 md:grid-cols-12">
              {featureImages.map((image, index) => <Reveal key={image} delay={(index % 3) * .08} className={`${index === 0 ? 'md:col-span-7' : index === 1 ? 'md:col-span-5' : index === 2 ? 'md:col-span-5' : index === 3 ? 'md:col-span-7' : index === 4 ? 'md:col-span-4' : index === 5 ? 'md:col-span-4' : 'md:col-span-4'}`}><article className={`feature-card group relative overflow-hidden rounded-[28px] ${index < 4 ? 'min-h-[350px]' : 'min-h-[300px]'} bg-slate-800`} data-testid={`card-feature-${index}`}><img src={image} alt={t.featureTitles[index]} loading="lazy" width="768" height="432" className="feature-image absolute inset-0 h-full w-full object-cover opacity-75" /><div className="absolute inset-0 bg-gradient-to-t from-[#101d30] via-[#101d30]/35 to-transparent" /><div className="relative flex h-full min-h-[inherit] flex-col justify-end p-6 sm:p-8"><div className="mb-auto flex h-9 w-9 items-center justify-center rounded-full border border-white/25 bg-white/10 text-cyan-200">{[Sparkles, HeartPulse, Flame, Dumbbell, Target, TrendingUp, Zap][index]({ size: 17 })}</div><p className="eyebrow text-cyan-200">0{index + 1}</p><h3 className="display mt-2 max-w-[330px] text-2xl font-bold leading-tight text-white">{t.featureTitles[index]}</h3><p className="mt-2 max-w-[390px] text-sm leading-5 text-slate-200">{t.featureBodies[index]}</p></div></article></Reveal>)}
            </div>
          </div>
        </section>
        ...
        <section className="relative overflow-hidden bg-[#e8f6fb] py-24 sm:py-32">
          <div className="container-wide grid items-center gap-14 lg:grid-cols-[1fr_.9fr]">
            <Reveal><span className="eyebrow text-cyan-700">{t.howKicker}</span><h2 className="display mt-5 whitespace-pre-line text-5xl font-extrabold leading-[.94] text-[#17263c] sm:text-7xl">{t.howTitle}</h2><p className="mt-7 max-w-md text-base leading-7 text-slate-600">{t.howBody}</p><a href="#join" className="group mt-8 inline-flex items-center gap-2 text-sm font-bold text-[#17263c]" data-testid="link-how-cta">{t.navCta}<ArrowRight size={17} className="transition-transform group-hover:translate-x-1" /></a></Reveal>
